...
const client_3 = require("@x402/evm/upto/client");
const client_4 = require("@x402/svm/exact/client");
...
const parseRpcUrlsFromEnv = () => {
    ...
    try {
        ...
        throw new Error("CDP_X402_RPC_URLS must be valid JSON, e.g. " +
            '\'{"eip155:137":"https://your-rpc-provider.example.com/polygon"}\'');
    }
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
        throw new Error("CDP_X402_RPC_URLS must be a JSON object mapping CAIP-2 network IDs to URL strings");
    }
    return Object.fromEntries(Object.entries(parsed).map(([network, url]) => {
...
const buildSvmRpcOverrides = (rpcUrlsByCaip2) => {
    const result = {};
    for (const [caip2, cfg] of Object.entries(rpcUrlsByCaip2)) {
...
const setupCdpSigners = async (client, config) => {
    ...
    const svmRpcOverrides = buildSvmRpcOverrides((resolvedRpcUrls ?? {}));
    const svmSigner = (0, account_signers_js_1.cdpSolanaAccountToSvmSigner)(svmAccount);
    (0, client_2.registerExactEvmScheme)(client, { signer: evmSigner, schemeOptions: evmRpcUrlsByChainId });
    (0, client_4.registerExactSvmScheme)(client, { signer: svmSigner });
    for (const [network, rpcUrl] of Object.entries(svmRpcOverrides)) {
        client.register(network, new client_4.ExactSvmScheme(svmSigner, { rpcUrl }));
    }
    /*
     ...
     */
    if (walletType !== "smart") {
        client.register("eip155:*", new client_3.UptoEvmScheme(evmSigner, evmRpcUrlsByChainId));
    }
