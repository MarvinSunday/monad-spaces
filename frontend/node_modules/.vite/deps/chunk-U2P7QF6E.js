...
function getAddress(config) {
  assert4(config);
  const hash5 = keccak2562(concat(fromString(accountDomain), padLeft2(config.salt ?? zeroSalt, 32), fromNumber(config.threshold, { size: 1 }), fromNumber(config.owners.length, { size: 1 }), ...config.owners.flatMap((owner) => [
    owner.owner,
    fromNumber(owner.weight, { size: 1 })
  ])));
  const account = from2(slice(hash5, 12, 32));
...
//# sourceMappingURL=chunk-U2P7QF6E.js.map
